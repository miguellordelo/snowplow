 ...
 */
case class YauaaEnrichment(cacheSize: Option[Int]) extends Enrichment {
  ...
  private val uaa: UserAgentAnalyzer = UserAgentAnalyzer
    .newBuilder()
    .build()
  if (cacheSize.isDefined) uaa.setCacheSize(cacheSize.get)

  // For unit testing
  def getCacheSize = uaa.getCacheSize
