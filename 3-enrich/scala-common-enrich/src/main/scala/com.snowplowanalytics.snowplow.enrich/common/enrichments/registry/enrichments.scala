...
final case class CurrencyConversionConf(
  ...
  override val filesToCache: List[(URI, String)] = Nil
  def enrichment[F[_]: Monad: CreateForex]: EitherT[F, String, CurrencyConversionEnrichment[F]] =
...
final case class IabConf(
  ...
  def enrichment[F[_]: Monad: CreateIabClient]: EitherT[F, String, IabEnrichment] =
